...
export class TypeScriptGenerator extends ClassGenerator {
  ...
  collectInterfaceClasses() {
    ...
    const enumCandidates =
      Config && Config.ENUMERABLE_CLASSES && Config.ENUMERABLE_CLASSES instanceof Set
        ...
        : Config && Config.INTERFACE_CLASSES && Config.INTERFACE_CLASSES instanceof Set
    ...
    enumCandidates.forEach((localName) => {
      const matched = Array.from(this.enumClasses || []).filter((ec) => {
        ...
        if (typeof this.ontology.isSubClassOf === 'function')
          return this.ontology.isSubClassOf(info.iri, localName);
        return false;
      });
      if (matched.length > 0)
        mappings.push({
          localName,
          enumClasses: matched,
          tsName: typeof this.pascalCase === 'function' ? this.pascalCase(localName) : localName,
        });
    });
    return mappings;
  }
  ...
  buildModelPreamble(enumFiles = [], interfaceImports = new Map(), needsArray = false) {
    const typedjsonImports = needsArray
    ...
    if (interfaceImports && interfaceImports.size > 0) {
      ...
      interfaceImports.forEach((iface, file) => {
        // Prefer importing from the display-local filename (e.g. Systeem.interface)
        // when present; fall back to the original superName-based filename.
        const displayLocal = String(iface).replace(/^I/, '');
        const displayIfaceFile = path.join(this.outputPath, `${displayLocal}.interface.ts`);
        const fallbackIfaceFile = path.join(this.outputPath, `${file}.interface.ts`);
        if (fs.existsSync(displayIfaceFile)) {
          preamble += `import type { ${iface} } from './${displayLocal}.interface';\n`;
        } else if (fs.existsSync(fallbackIfaceFile)) {
  ...
  writeIndexFile(outputPath, enumFiles = []) {
    ...
    const interfaceFilesOnDisk = fs
      .readdirSync(outputPath)
      .filter((f) => f.endsWith('.interface.ts'));
    interfaceFilesOnDisk.forEach((f) =>
      indexLines.push(`export * from './${f.replace(/\.ts$/, '')}';`)
    );
  ...
  fixModelImportSpacing(outputPath) {
    ...
    modelFiles.forEach((mf) => {
      ...
      if (fixed !== c) fs.writeFileSync(p, fixed, 'utf8');
    });
  }
  ...
  cleanupUnusedImports(outputPath) {
    ...
    modelFiles.forEach((mf) => {
      ...
      const contentWithoutImports = lines
        .filter((_, i) => !importLines.some((il) => il.idx === i))
        .join('\n');
      let modified = false;
  ...
  _cleanupImportsInContent(content) {
    ...
    importLines.forEach((il) => {
      withoutImports = withoutImports.replace(il.full, '');
    });
    let out = content;
    importLines.reverse().forEach((il) => {
      const names = il.spec
  ...
  buildModelImports(modelImportsSet, currentClassName) {
    ...
    const names = Array.from(modelImportsSet)
      .filter((n) => n && n !== currentClassName)
      .sort();
  ...
  writeSharedInterfaces(
    sharedSupers,
    sharedInterfaceNames,
    usedSharedInterfaces,
    outputPath,
    classToSupers = new Map()
  ) {
    ...
    sharedSupers.forEach((superName) => {
      ...
      const displayLocal = ifaceName ? String(ifaceName).replace(/^I/, '') : superName;
      const ifaceFile = path.join(outputPath, `${displayLocal}.interface.ts`);
      const { props, imports: ifaceImports } = this.renderSharedInterfaceProps(
        superName,
        sharedInterfaceNames,
        classToSupers
      );
      ...
      // Also write alias under the original superName to preserve compatibility
      const aliasFile = path.join(outputPath, `${superName}.interface.ts`);
      if (aliasFile !== ifaceFile) fs.writeFileSync(aliasFile, ifaceContent, 'utf8');
    });
  }

  tsTypeForAttribute(attr) {
    if (attr.type === 'enum') {
      if (attr && attr.enumClass)
        return typeof this.pascalCase === 'function'
          ? this.pascalCase(attr.enumClass)
          : String(attr.enumClass);
  ...
  generate() {
    ...
    this.writeSharedInterfaces(
      sharedState.sharedSupers,
      sharedState.sharedInterfaceNames,
      sharedState.usedSharedInterfaces,
      this.outputPath,
      sharedState.classToSupers
    );

    // Emit models
    const modelIndexEntries = this._emitModels(
      classNames,
      enumState.enumFiles,
      enumState.otherEnumClasses,
      enumState.interfaceEnumMappings,
      sharedState
    );
    ...
    enumState.enumFiles.forEach((e) =>
      indexLines.push(`export * from './${path.basename(e.file, '.ts')}';`)
    );
    const interfaceFilesOnDisk = fs
  ...
  _emitEnums(classNames) {
    ...
    const otherEnumClasses = Array.from(this.enumClasses).filter(
      (ec) => !mappedEnumClasses.has(ec)
    );
  ...
  _computeSharedStructures(classNames) {
    ...
    const usedSharedInterfaces = this.computeUsedSharedInterfaces(
      classNames,
      classToSupers,
      sharedInterfaceNames
    );
  ...
  _emitModels(classNames, enumFiles, otherEnumClasses, interfaceEnumMappings, sharedState) {
    ...
    const { classToSupers, sharedInterfaceNames, usedSharedInterfaces, relPropertyMap } =
      sharedState;
    classNames.forEach((className) => {
      ...
      // Prefer the business/display name for generated TS class; use a
      // PascalCase filename (no spaces) to avoid filesystem names with spaces.
      const preferredName = this.getPreferredBusinessName
        ? this.getPreferredBusinessName(className)
        : className;
      // Determine whether this class is internal or external and choose
      // exported class name and filename accordingly.
      let classLocal = sourceLocalName || null;
      let classTsName;
      let fileBase;
      const classInfoLocal =
        classLocal && this.ontology && this.ontology.classes
          ? this.ontology.classes.get(classLocal)
          : null;
      if (classInfoLocal) {
        if (classInfoLocal.external) {
          // external/imported class: prefer business name for display and filename
          const biz =
            this.getBusinessClassName && this.getBusinessClassName(classLocal)
              ? this.getBusinessClassName(classLocal)
              : classLocal;
          classTsName = pascal(biz);
          fileBase = biz;
        } else {
          // internal class: use ontology local name for filename and exported class
          classTsName = pascal(classLocal);
          fileBase = classLocal;
        }
      } else {
        // class not present in ontology map: fall back to business/display name
        classTsName = pascal(preferredName);
        fileBase = classTsName;
      }
      const fileName = path.join(this.outputPath, `${fileBase}.model.ts`);
      ...
      if (implementsIface) {
        const superEntry = [...sharedInterfaceNames.entries()].find(
          ([, val]) => val === implementsIface
        );
        if (superEntry) {
      ...
      attrs.forEach((attr) => {
        ...
        if (attr.type === 'enum') {
          ...
          if (attr.enumClass) {
            ...
            const enumClass = otherEnumClasses.find(
              (ec) =>
                ec === attr.comment || ec === attr.propertyIri || pascal(ec) === pascal(attr.name)
            );
      ...
      attrs.forEach((attr) => {
        // prefer full property IRI for JSON member names. Fall back to any
        // explicit jsonName or attribute name when an IRI is not available.
        const jsonName =
          (attr.propertyIri && typeof attr.propertyIri === 'string' && attr.propertyIri.length > 0)
            ? attr.propertyIri
            : attr.jsonName && typeof attr.jsonName === 'string' && attr.jsonName.length > 0
            ? attr.jsonName
            : attr.name && String(attr.name).length > 0
            ? attr.name
            : '';
        ...
        if (mapped) {
          ...
          if (baseType === 'Date') mappedMemberCtor = 'Date';
            ...
            mappedMemberCtor = baseType;
          if (attr.isPrimaryKey) content += `  // PK (primary key from ontology/hydra IRI)\n`;
          ...
          if (!attr.synthetic && attr.propertyIri) {
            officialName =
              this.ontology.getBusinessNameForProperty(attr.propertyIri, className) ||
        ...
        if (implementsIface) {
          ...
          if (superEntry) {
            ...
            if (superInfo) {
              const superAttrs =
                this.utils.deriveAttributes(superInfo, this.utils.enumClasses, superName) || [];
              const matching = superAttrs.find((sa) => {
                if (!sa) return false;
                if (sa.name === attr.name) return true;
                if (sa.propertyIri && attr.propertyIri && sa.propertyIri === attr.propertyIri)
                  return true;
                try {
                  const saLocal = sa.propertyIri
                    ? this.ontology.extractLocalName(sa.propertyIri)
                    : sa.name;
                  const aLocal = attr.propertyIri
                    ? this.ontology.extractLocalName(attr.propertyIri)
                    : attr.name;
              ...
              if (matching) {
                ...
                if (matching.type === 'enum') {
                  const enumClass = otherEnumClasses.find(
                    (ec) =>
                      ec === matching.comment ||
                      ec === matching.propertyIri ||
                      pascal(ec) === pascal(matching.name)
                  );
                  if (enumClass) st = pascal(enumClass);
                }
                if (
                  matching.isForeignKey &&
                  Array.isArray(matching.targetClasses) &&
                  matching.targetClasses.length === 1
                ) {
                  const t0 = matching.targetClasses[0];
                  if (classNames.includes(t0)) st = 'string';
                }
                const matchingMax =
                  typeof matching.maxCardinality === 'number' && matching.maxCardinality >= 0
                    ? matching.maxCardinality
                    : undefined;
                const isArraySuper =
                  (typeof matchingMax === 'number' && matchingMax !== 1) ||
                  (matchingMax === undefined &&
                    matching.minCardinality !== 1 &&
                    !matching.isPrimaryKey &&
                    matching.isForeignKey);
        ...
        if (
          ...
          attr.targetClasses.length >= 1
        ) {
          const targets = attr.targetClasses.filter((t) => classNames.includes(t));
          const localPropName = attr.propertyIri
            ? this.ontology.extractLocalName(attr.propertyIri)
            : attr.name;
          const relMapped =
            relPropertyMap.get(`${className}|${localPropName}`) ||
            relPropertyMap.get(`${className}|${attr.name}`);
          if (relMapped) {
            if (/^I[A-Z]/.test(relMapped)) {
              // For synthetic attributes prefer the interface type (e.g. ISystem).
              // Otherwise, fall back to preferring a visible concrete implementor
              // as before.
              if (attr && attr.synthetic) {
                baseType = relMapped + (isArray ? '[]' : '');
                const ifaceLocal = String(relMapped).replace(/^I/, '');
                interfaceImports.set(ifaceLocal, relMapped);
                targets.length = 0;
              } else {
                // Prefer a visible concrete implementor for this interface when available
                const concrete = this.findConcreteImplementorForInterface(
                  relMapped,
                  classToSupers,
                  sharedInterfaceNames,
                  classNames
                ...
                if (concrete) {
                  baseType = pascal(concrete) + (isArray ? '[]' : '');
              ...
              const sharedIfaceForRel = this.findSharedInterfaceForTargets(
                [relMapped],
                classToSupers,
                sharedInterfaceNames
              );
          ...
          if ((!targets || targets.length === 0) && attr.propertyIri) {
            ...
            const mapped =
              relPropertyMap.get(`${className}|${localProp}`) ||
              relPropertyMap.get(`${className}|${attr.name}`);
            if (mapped) {
          ...
          if (sharedIface) {
            // Special-case: if the shared interface maps to the configured
            // System super-class, prefer the interface type even when a
            // concrete implementor exists. This covers display-name changes
            // (e.g. ISysteem).
            ...
            if (superName === 'System') {
              ...
              if (superName) interfaceImports.set(superName, sharedIface);
            } else {
              // prefer a visible concrete implementor for the shared interface
              const concreteImpl = this.findConcreteImplementorForInterface(
                sharedIface,
                classToSupers,
                sharedInterfaceNames,
                classNames
              ...
              if (concreteImpl) {
                baseType = pascal(concreteImpl);
                modelImports.add(concreteImpl);
              } else {
                const superEntry = [...sharedInterfaceNames.entries()].find(
                  ([, v]) => v === sharedIface
                );
                const superName = superEntry ? superEntry[0] : null;
                const ifaceFilePath = superName
                  ? path.join(this.outputPath, `${superName}.interface.ts`)
                  : null;
          ...
          const explicitFromComment = (attr.comment || '')
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean)
            .filter((tn) => classNames.includes(tn));
          ...
          const explicitTargets = Array.isArray(attr.targetClasses)
            ? attr.targetClasses.filter((t) => classNames.includes(t))
            : [];
          const hasInternalTarget = explicitTargets.some((tn) => {
            const ti = this.ontology.classes.get(tn);
          ...
          if (anyAgent && !hasInternalTarget) {
            if (!baseType || baseType === 'string' || baseType === 'object') {
              ...
              const agentEntryLocal =
                Config && Config.INTERFACE_CLASSES && Config.INTERFACE_CLASSES instanceof Set
                  ? Array.from(Config.INTERFACE_CLASSES).find((k) => /agent/i.test(k)) || 'Agent'
                  : 'Agent';
              const agentEntry = [...sharedInterfaceNames.entries()].find(
                ([s, iface]) =>
                  s === agentEntryLocal ||
                  (this.ontology.classes.get(s) &&
                    this.ontology.isSubClassOf(this.ontology.classes.get(s).iri, agentEntryLocal))
              );
              if (agentEntry) {
              ...
              const attrMax =
                typeof attr.maxCardinality === 'number' && attr.maxCardinality >= 0
                  ? attr.maxCardinality
                  : undefined;
              const attrIsArray =
                (typeof attrMax === 'number' && attrMax !== 1) ||
                (attrMax === undefined &&
                  attr.minCardinality !== 1 &&
                  !attr.isPrimaryKey &&
                  attr.isForeignKey);
              if (agentIfaceName) {
        ...
        let localProp = null;
        if (attr.jsonName && typeof this.ontology.extractLocalName === 'function')
          localProp = this.ontology.extractLocalName(attr.jsonName);
        else if (attr.propertyIri && typeof this.ontology.extractLocalName === 'function')
          localProp = this.ontology.extractLocalName(attr.propertyIri);
        ...
        if (
          ...
          attr.synthetic &&
          Array.isArray(attr.targetClasses) &&
          attr.targetClasses.includes('System')
        ) {
          // Prefer the configured shared interface name for 'System' when available
          const ifaceName = sharedInterfaceNames.get('System') || `I${pascal('System')}`;
          baseType = ifaceName;
          interfaceImports.set('System', ifaceName);
        }

        // If baseType references a concrete name that corresponds to a
        // shared/display interface (e.g. `Systeem`), prefer the interface
        // type here so decorators import the interface instead of the model.
        try {
          const elemTypeCheck = String(baseType).replace(/\[\]$/, '');
          for (const [superName, ifaceName] of (sharedInterfaceNames || []).entries ? sharedInterfaceNames.entries() : []) {
            const displayLocal = ifaceName ? String(ifaceName).replace(/^I/, '') : superName;
            const dp = this.pascalCase ? this.pascalCase(displayLocal) : displayLocal;
            const ep = this.pascalCase ? this.pascalCase(elemTypeCheck) : elemTypeCheck;
            if (dp === ep) {
              const isArraySuffix = String(baseType).endsWith('[]') ? '[]' : '';
              baseType = ifaceName + isArraySuffix;
              interfaceImports.set(superName, ifaceName);
              break;
            }
          }
        } catch (e) {
        ...
        if (baseType === 'Date') memberCtor = 'Date';
          ...
          const isEnum =
            Array.isArray(localEnumFiles) &&
            localEnumFiles.length > 0 &&
            localEnumFiles.some((e) => e.name === String(baseType));
          if (isEnum) memberCtor = `() => ${baseType}`;
          else memberCtor = baseType;
        }
        const ifaceMatch = String(baseType).match(/I[A-Z][A-Za-z0-9_]*/);
        if (ifaceMatch) memberCtor = 'Object';

        const requiredInClass = Boolean(
          attr.isPrimaryKey || (typeof attr.minCardinality === 'number' && attr.minCardinality >= 1)
        );
        ...
        try {
          ...
          const isEnumElem =
            Array.isArray(localEnumFiles) &&
            localEnumFiles.length > 0 &&
            localEnumFiles.some((e) => e.name === elemType);
          if (isEnumElem) decoratorCtor = `() => ${elemType}`;
        } catch (e) {
          /* fall back to existing decoratorCtor */
        }
        if (isArray) {
          if (attr.isPrimaryKey) content += `  // PK (primary key from ontology/hydra IRI)\n`;
      ...
      const updatedPreamble =
        this.buildModelPreamble(localEnumFiles, interfaceImports, needsArray) + '\n';
      ...
      const tplForMethod =
        typeof this.ontology.getIriTemplateForClass === 'function'
          ? this.ontology.getIriTemplateForClass(
              classInfo && classInfo.iri ? classInfo.iri : className
            )
          : null;
      if (tplForMethod && tplForMethod.template) {
        ...
        (tplForMethod.mappings || []).forEach((m) => {
          const local =
            m.propertyIri && typeof this.ontology.extractLocalName === 'function'
              ? this.ontology.extractLocalName(m.propertyIri)
              : null;
      ...
      // If a shared/display interface corresponds to a concrete model import,
      // prefer the interface: convert modelImport -> interfaceImport when
      // the display-local name matches a model import (e.g. Systeem).
      try {
        if (modelImports && modelImports.size > 0 && sharedInterfaceNames && sharedInterfaceNames.size > 0) {
          for (const m of Array.from(modelImports)) {
            const mPas = this.pascalCase ? this.pascalCase(m) : m;
            for (const [superName, ifaceName] of sharedInterfaceNames.entries()) {
              const displayLocal = ifaceName ? String(ifaceName).replace(/^I/, '') : superName;
              const dPas = this.pascalCase ? this.pascalCase(displayLocal) : displayLocal;
              if (mPas === dPas) {
                // Replace model import with interface import for this shared super
                modelImports.delete(m);
                interfaceImports.set(superName, ifaceName);
                break;
              }
            }
        ...
        // Also, if an interface import already exists for a given local name,
        // remove any remaining model import collisions to prefer interfaces.
        if (interfaceImports && interfaceImports.size > 0 && modelImports && modelImports.size > 0) {
          for (const [ifaceLocal] of interfaceImports) {
            if (modelImports.has(ifaceLocal)) modelImports.delete(ifaceLocal);
            const pas = this.pascalCase ? this.pascalCase(ifaceLocal) : ifaceLocal;
            for (const m of Array.from(modelImports)) {
              const mp = this.pascalCase ? this.pascalCase(m) : m;
              if (mp === pas) modelImports.delete(m);
            }
          }
        }
      } catch (e) {
          ...
          if (superEntry) {
            ...
            if (fs.existsSync(ifaceFile)) {
              ...
              while ((m = propRe.exec(ifaceContent)) !== null) {
                ...
                if (!prop) continue;
                const propTypeRe = new RegExp(
                  `(\\b${prop}\\b\\s*[!?]:\\s*)([A-Za-z0-9_\\[\\]]+)`,
                  'g'
                );
                content = content.replace(propTypeRe, `$1${itype}`);
                const decoRe = new RegExp(
                  `@json(Member|ArrayMember)\\(([^)]*)\\)\\s*\\n\\s*${prop}\\b`,
                  'g'
                );
                  ...
                  classAttr = (attrs || []).find((a) => {
                    ...
                    try {
                      if (a.propertyIri) {
                        ...
                        const aBiz = this.ontology.getBusinessNameForProperty
                          ? this.ontology.getBusinessNameForProperty(a.propertyIri, className)
                          : null;
                        if (aBiz === prop) return true;
                        // Finally, compare derived official attribute name
                        const aOfficial =
                          aBiz ||
                          (a.propertyIri ? this.ontology.extractLocalName(a.propertyIri) : a.name);
                  ...
                  content = content.replace(decoRe, (all, p1, args) => {
                    ...
                    if (/^I[A-Z]/.test(elem)) ctor = 'Object';
                      ...
                      const isEnumElem =
                        Array.isArray(localEnumFiles) &&
                        localEnumFiles.length > 0 &&
                        localEnumFiles.some((e) => e.name === elem);
  ...
  ${prop}`;
                  ...
                  try {
                    if (classAttr) {
                      const typeCleanupRe = new RegExp(
                        `(\\b${prop}\\b\\s*[!?]:\\s*([A-Za-z0-9_]+))\\[\\];`,
                        'g'
                      );
      ...
      const ifaceMatches = Array.from(content.matchAll(/\b(I[A-Z][A-Za-z0-9_]*)\b/g)).map(
        (m) => m[1]
      );
      ifaceMatches.forEach((iface) => {
        ...
        if (!hasImport && !fs.existsSync(ifaceFile)) {
          content = content.replace(
            new RegExp(`@json(Member|ArrayMember)\\(${iface}`, 'g'),
            '@json$1(String'
          );
      ...
      try {
        content = this._cleanupImportsInContent(content);
      } catch (e) {}
      content = content.replace(/^[\s\n\r]+/, '');
      // Emit IRI template comment if available via hydra:search -> hydra:IriTemplate
      let iriComment = '';
      try {
        const tpl = this.ontology.getIriTemplateForClass(
          classInfo && classInfo.iri ? classInfo.iri : className
        );
        if (tpl && tpl.template) {
          ...
          if (Array.isArray(tpl.mappings) && tpl.mappings.length > 0) {
            tpl.mappings.forEach((m) => {
              const propShort = m.propertyIri
                ? this.ontology.extractLocalName(m.propertyIri) || m.propertyIri
                : m.propertyIri;
              iriComment += `// Mapping: {${m.variable}} -> ${propShort}${
                m.required ? ' (required)' : ''
              }\n`;
            });
          }
          ...
          if (biz && biz !== mn) {
            ...
            if (fs.existsSync(bizFile)) {
              ...
              const importRe = new RegExp(
                `import \\{\\s*${pasMn}\\s*\\} from '\\.\\/${mn}\\.model';`,
                'g'
              );
      ...
      // Ensure exported class name matches the ontology local filename when
      // the fileBase corresponds to an ontology class. This fixes cases where
      // the generated class used a business/display name while the filename
      // is the local ontology name.
      try {
        const desiredClassName = pascal(fileBase);
        let written = fs.readFileSync(fileName, 'utf8');
        written = written.replace(
          /export class\s+[A-Za-z0-9_]+/,
          `export class ${desiredClassName}`
        );
        fs.writeFileSync(fileName, written, 'utf8');
      } catch (e) {
        /* ignore post-write patch errors */
      }
      try {
        if (Array.isArray(enumFiles) && enumFiles.length > 0) {
          let written = fs.readFileSync(fileName, 'utf8');
      ...
      try {
        for (const [localName, info] of this.ontology.classes) {
          try {
            ...
            /* ignore per-class alias errors */
          }
        }
      } catch (e) {
        /* ignore alias generation errors */
      }
      indexEntries.push(`export * from './${fileBase}.model';`);
    });
    return indexEntries;
  }
