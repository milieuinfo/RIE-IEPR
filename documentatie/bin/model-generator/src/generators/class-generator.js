...
export class ClassGenerator extends BaseGenerator {
  ...
  isAttributeMultiValued(attr) {
    ...
    const local =
      attr.propertyIri && this.ontology && typeof this.ontology.extractLocalName === 'function'
    ...
    if (
      local === 'label' ||
      biz === 'benaming' ||
      String(attr.propertyIri).endsWith('/label') ||
      String(attr.propertyIri).endsWith('#label')
    )
      return false;
    ...
    superAttrs.forEach((attr) => {
      ...
      if (attr.type === 'date' || attr.type === 'datetime') t = 'Date';
        ...
        if (
          !enumClass &&
          this.enumClasses &&
          typeof this.enumClasses[Symbol.iterator] === 'function'
        ) {
          enumClass = Array.from(this.enumClasses).find(
            (ec) =>
              ec === attr.comment ||
              ec === attr.propertyIri ||
              this.pascalCase(ec) === this.pascalCase(attr.name)
          );
        }
        // Only accept short/safe comments as enum names (avoid long human-readable sentences)
        const isSafeEnumLabel = (c) =>
          typeof c === 'string' &&
          c.length > 0 &&
          c.length <= 30 &&
          !/\s/.test(c) &&
          /^[A-Za-z0-9_\-]+$/.test(c);
        const enumName = enumClass
          ? this.pascalCase(enumClass)
          : isSafeEnumLabel(attr.comment)
          ? this.pascalCase(attr.comment)
          : this.pascalCase(attr.name);
        t = enumName || 'string';
      }
      ...
      if (attr.isForeignKey && Array.isArray(attr.targetClasses) && attr.targetClasses.length > 0) {
        ...
        if (explicitInternal.length === 1) {
          ...
          // prefer a shared interface for the explicit internal target when available
          const iface = this.findSharedInterfaceForTargets(
            [single],
          ...
          if (iface) {
            t = iface;
            const localName = String(iface).replace(/^I/, '');
            imports.add(localName);
          } else {
            const display =
              this.getDisplayName(single) || (this.pascalCase ? this.pascalCase(single) : single);
            t = display;
            imports.add(display);
          }
        } else {
      ...
      if (classToSupers && typeof classToSupers.forEach === 'function') {
        classToSupers.forEach((supers, cls) => {
          if (Array.isArray(supers) && supers.includes(superName) && cls !== superName)
            implementers.push(cls);
        });
      }
      for (const impl of implementers) {
        ...
        const matching = implAttrs.find((a) => {
          ...
          const attrLocal = attr.propertyIri
            ? this.ontology.extractLocalName(attr.propertyIri)
            : attr.name;
          if (aLocal && attrLocal && aLocal === attrLocal) return true;
          const aBiz = this.ontology.getBusinessNameForProperty
            ? this.ontology.getBusinessNameForProperty(a.propertyIri, impl)
            : null;
          const attrBiz = this.ontology.getBusinessNameForProperty
            ? this.ontology.getBusinessNameForProperty(attr.propertyIri, impl)
            : null;
          if (aBiz && attrBiz && aBiz === attrBiz) return true;
          return false;
        });
        if (matching && this.isAttributeMultiValued(matching)) {
          isArray = true;
          break;
        }
      }
      ...
      if (!existing) {
        ...
        if (existing.name.endsWith('Id') && !propName.endsWith('Id')) {
          propMap.set(base, {
            name: propName,
            type: newIsIface ? t : existingIsIface ? existing.type : t,
            comment: attr.comment || null,
          });
        } else if (newIsIface && !existingIsIface) {
          propMap.set(base, { name: propName, type: t, comment: attr.comment || null });
        } else {
          if (existing.type === 'object' && t !== 'object')
            propMap.set(base, { name: propName, type: t, comment: attr.comment || null });
        }
      }
    });
    ...
    byNorm.forEach((list) => {
      if (list.length === 1) props.push(list[0]);
        ...
        if (iface) props.push(iface);
          ...
          const nonEnum = list.find(
            (l) => !/[A-Za-z0-9_]+\[?\]?/.test(l.type) || !l.type.match(/^[A-Z][a-zA-Z0-9_]*/)
          );
  ...
  applyPropertyRenderOverride(
    ...
    if (mapped) {
      ...
      const biz = this.ontology.getBusinessNameForProperty
      ...
      name =
        typeof rawName === 'string' && rawName.length > 0
          ...
          : rawName || '';
      if (
        attr.propertyIri === 'rdfs:label' ||
        attr.propertyIri === `${Config.NAMESPACES.rdfs}label`
      )
        type = 'string';
      else if (attr.type === 'enum') {
    ...
    if (attr.isForeignKey && Array.isArray(attr.targetClasses) && attr.targetClasses.length > 0) {
      ...
      if (explicitInternal.length === 1) {
        ...
        if (iface) {
          ...
          const display =
            this.getDisplayName(target) || (this.pascalCase ? this.pascalCase(target) : target);
          type = display;
        }
      } else {
        const targets = attr.targetClasses.filter((t) => {
          const isRelevant =
            this.ontology.isRelevantClassName && this.ontology.isRelevantClassName(t);
      ...
      if (
        ...
        if (info && info.iri) {
          ...
          if (subjects.size > 0) {
            vals = [];
            subjects.forEach((s) => {
              const labelQuad = (this.ontology.store.getQuads(s, null, null) || []).find((lq) =>
                String((lq.predicate && lq.predicate.value) || '')
                  .toLowerCase()
                  .includes('label')
              );
              if (labelQuad && labelQuad.object && labelQuad.object.value)
                vals.push(labelQuad.object.value);
              else if (s && s.termType === 'NamedNode') {
                const local = this.ontology.extractLocalName
                  ? this.ontology.extractLocalName(s.value)
                  : s.value.split(/[#\\/]/).pop();
  ...
  /**
   * Given an interface display name (e.g. `IAgent`) attempt to find a
   * single visible concrete implementor (prefer non-external classes).
   * Returns the concrete class local name or null if none found.
   */
  findConcreteImplementorForInterface(
    ifaceName,
    classToSupers,
    sharedInterfaceNames,
    visibleClasses
  ) {
    if (!ifaceName || !sharedInterfaceNames || !classToSupers) return null;
    // find superName that maps to this iface
    let superName = null;
    for (const [s, iface] of sharedInterfaceNames.entries()) {
      if (iface === ifaceName) {
        superName = s;
        break;
      }
    ...
    const visibles = Array.isArray(visibleClasses)
      ? visibleClasses
      : visibleClasses && typeof visibleClasses.has === 'function'
      ? Array.from(visibleClasses)
      : Array.from(this.computeVisibleClasses() || []);
    let fallback = null;
    for (const [cn, supers] of classToSupers.entries()) {
      if (!Array.isArray(supers) || !supers.includes(superName)) continue;
      if (cn === superName) continue;
      // prefer visible classes
      if (visibles.includes(cn)) {
        const info = this.ontology.classes.get(cn);
        if (info && !info.external) return cn; // prefer internal
        if (!fallback) fallback = cn; // remember external fallback
      }
    }
    return fallback;
  }
