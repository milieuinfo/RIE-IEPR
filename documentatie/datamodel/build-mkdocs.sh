...
pip install -q --break-system-packages --no-index --find-links "$(dirname "${BASH_SOURCE[0]}")/wheels" -r requirements-mkdocs.txt >/dev/null 2>&1 || pip install -q --break-system-packages -r requirements-mkdocs.txt >/dev/null 2>&1 || echo "pip install skipped / already satisfied"
